...
{
    public class SpeakerRegionAggregationProvider : BaseAggregationProvider
    {
        private readonly GosDbContext dbContext;

        public SpeakerRegionAggregationProvider(IAggregatorFactory aggregatorFactory, GosDbContext dbContext)
            : base(aggregatorFactory)
        {
            this.dbContext = dbContext;
        }

        public override AggregationType Type => AggregationType.SpeakerRegion;
        ...
        protected override List<AggregationItem> GetItems(IDictionary<string, int> items)
        {
            ...
            var types = dbContext.SpeakerRegions.Include(e => e.Translations).Where(tt => ids.Contains(tt.Id)).ToList();
