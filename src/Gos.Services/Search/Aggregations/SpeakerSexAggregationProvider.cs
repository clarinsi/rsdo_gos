...
{
    public class SpeakerSexAggregationProvider : BaseAggregationProvider
    {
        private readonly GosDbContext dbContext;

        public SpeakerSexAggregationProvider(IAggregatorFactory aggregatorFactory, GosDbContext dbContext)
            : base(aggregatorFactory)
        {
            this.dbContext = dbContext;
        }

        public override AggregationType Type => AggregationType.SpeakerSex;
        ...
        protected override List<AggregationItem> GetItems(IDictionary<string, int> items)
        {
            ...
            var types = dbContext.SpeakerSexes.Include(e => e.Translations).Where(tt => ids.Contains(tt.Id)).ToList();

            return (from item in items
                let id = int.Parse(item.Key)
                join sex in types on id equals sex.Id
                orderby item.Value descending, sex.Title
